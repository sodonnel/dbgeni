...
The skeleton config file generated by the new or new-config commands is given below:
    ...
    # Valid values are oracle, mysql, sqlite however this is not validated at runtime
    ...
    #   database 'testdb.sqlite # this must be here, or it will error. For Oracle, this is the TNS Name
    #   password ''             # If this value is missing, it will be promoted for when needed
    # }
...
This contains details about each environment dbgeni can connect to. In this release, the only useful parameters are database, username, password, hostname and pport. In future releases other parameters can be defined here and used as parameters in SQL scripts.
...
There can be many environment definitions, but each should have a unique name. The idea is to create an environment section for each database you want to apply migrations against. An environment is defined using the syntax below:
    ...
    environment('development') {
      ...
      hostname 'IP address of mysql server - ignored for Oracle and sqlite'
      port     'Port mysql server runs on - ignored for Oracle and sqlite'
    }
...
When coding a procedure, function, package or trigger for Oracle, the create or replace syntax should be used. There should also be a slash character after the procedure definition to allow sqlplus execute the code. For example:
...
Often permissions to execute the procedure need to be granted to various users after it is compiled. Such privileges can be included in the file along with the procedure. For example:
...
MySQL handles stored procedures differently from Oracle. If a procedure has a syntax error, it will not be installed on the database at all. However, if a procedure refers to another procedure or table that does not exist, MySQL will not error at compile time, and no errors will be reported. Therefore if errors are encountered when applying MySQL stored procedures, dbgeni will stop processing just like with migrations.
