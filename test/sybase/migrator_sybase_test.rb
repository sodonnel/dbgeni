...
#### code tests
  ...
  def test_no_code_errors_reported_for_good_procedure
    code = helper_sybase_good_procedure_file
    assert_nothing_raised do
  ...
  def test_code_errors_reported_for_bad_procedure
    code = helper_sybase_bad_procedure_file
    assert_raises DBGeni::MigrationContainsErrors do
      @migrator.compile(code)
    end
  ...
  def test_procedure_is_dropped_successfully
    ...
    assert_nothing_raised do
      @migrator.compile(code)
      assert_equal(1, @connection.execute("select name from sysobjects where name = 'proc1'").length)
      @migrator.remove(code)
    end
    assert_equal(0, @connection.execute("select name from sysobjects where name = 'proc1'").length)
  end

  def test_no_code_errors_reported_for_good_function
    code = helper_sybase_good_function_file
    assert_nothing_raised do
      @migrator.compile(code)
    end
    assert_nil(@migrator.code_errors)
  ...
  def test_code_errors_reported_for_bad_function
    code = helper_sybase_bad_function_file
    assert_raises DBGeni::MigrationContainsErrors do
      @migrator.compile(code)
    end
    assert_not_nil(@migrator.code_errors)
  ...
  def test_function_is_dropped_successfully
    ...
    assert_nothing_raised do
      @migrator.compile(code)
      assert_equal(1, @connection.execute("select name from sysobjects where name = 'func1'").length)
      @migrator.remove(code)
    end
    assert_equal(0, @connection.execute("select name from sysobjects where name = 'func1'").length)
  end

  def test_no_code_errors_reported_for_good_trigger
    code = helper_sybase_good_trigger_file
    ...
    assert_equal(1, @connection.execute("select name from sysobjects where name = 'trg1'").length)
  end

  def test_code_errors_reported_for_bad_trigger
    code = helper_sybase_bad_trigger_file
    @connection.execute('create table foo (c1 varchar(10))')
    assert_raises DBGeni::MigrationContainsErrors do
      @migrator.compile(code)
    end
  ...
  def test_trigger_is_dropped_successfully
    ...
    assert_equal(0, @connection.execute("select name from sysobjects where name = 'trg1'").length)
  end
