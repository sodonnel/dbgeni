...
class TestCLIDMLMigrations < Test::Unit::TestCase
  ...
  def test_errors_when_env_specified_that_does_not_exist
    helper_sqlite_multiple_environment_file
    response = `#{CLI} dmls list -c  #{TEMP_DIR}/sqlite.conf -e foobar`
  ...
  def test_can_list_migrations_when_none
    @dbs.each do |db|
      response = Kernel.system("#{CLI} dmls list -c #{TEMP_DIR}/#{db}.conf")
  ...
  def test_can_list_migrations_when_some
    @dbs.each do |db|
      self.send("helper_good_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls list -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_can_list_outstanding_migrations_when_none
    @dbs.each do |db|
      response = Kernel.system("#{CLI} dmls outstanding -c #{TEMP_DIR}/#{db}.conf")
  ...
  def test_can_list_applied_migrations_when_none
    @dbs.each do |db|
      response = Kernel.system("#{CLI} dmls applied -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_can_list_applied_migrations_when_some
    @dbs.each do |db|
      self.send("helper_good_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply all -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      response = Kernel.system("#{CLI} dmls applied -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  ...
  def test_apply_next_migration_when_none
    @dbs.each do |db|
      response = `#{CLI} dmls apply next -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/There are no outstanding dml migrations to apply/, response)
    end
  end
  ...
  def test_apply_next_migration_when_some
    @dbs.each do |db|
      self.send("helper_good_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply next -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_next_migration_with_errors
    @dbs.each do |db|
      self.send("helper_bad_#{db}_dml".intern)
      response = `#{CLI} dmls apply next -c #{TEMP_DIR}/#{db}.conf`
  ...
  def test_apply_next_migration_with_errors_and_force
    @dbs.each do |db|
      self.send("helper_bad_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply next -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_all_migrations_when_none
    @dbs.each do |db|
      response = `#{CLI} dmls apply all -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/There are no outstanding dml migrations to apply/, response)
    end
  end
  ...
  def test_apply_all_migrations_with_errors_and_force
    @dbs.each do |db|
      self.send("helper_bad_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply all -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_specific_good_migration
    @dbs.each do |db|
      self.send("helper_good_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_specific_bad_migration_with_force
    @dbs.each do |db|
      self.send("helper_bad_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response, "for #{db}")
    end
  end

  def test_apply_specific_migration_when_not_exist
    @dbs.each do |db|
      response = `#{CLI} dmls apply 201108190000::not_there -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/The dml migration file .* does not exist/, response, db)
    end
  end
  ...
  def test_apply_specific_migration_that_has_already_been_applied
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response, db)
      response = `#{CLI} dmls apply 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/The dml migration is already applied/, response, db)
    end
  end
  ...
  def test_apply_until_good_migration
    @dbs.each do |db|
      self.send("helper_good_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply until 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_until_bad_migration_force_on
    @dbs.each do |db|
      self.send("helper_bad_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls apply until 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_until_migration_that_does_not_exist
    @dbs.each do |db|
      response = `#{CLI} dmls apply until 201108200000::test_migration -c #{TEMP_DIR}/#{db}.conf`
  ...
  def test_apply_until_migration_with_invalid_name
    @dbs.each do |db|
      response = `#{CLI} dmls apply until 2011000000::test_migration -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/is not a valid dml migration name/, response)
    end
  end

  def test_apply_until_migration_missing_name
    @dbs.each do |db|
      response = `#{CLI} dmls apply until -c #{TEMP_DIR}/sqlite.conf`
      assert_match(/A dml migration name must be specified/, response)
    end
  end

  def test_apply_until_migration_many_migrations
    @dbs.each do |db|
      self.send("helper_many_good_#{db}_dmls".intern, 4)
      response = Kernel.system("#{CLI} dmls apply until 201108190002::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_until_migration_many_bad_migrations
    @dbs.each do |db|
      self.send("helper_many_bad_#{db}_dmls".intern, 4)
      response = Kernel.system("#{CLI} dmls apply until 201108190002::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(false, response)
    end
  end
  ...
  def test_apply_until_migration_many_bad_migrations_force_on
    @dbs.each do |db|
      self.send("helper_many_bad_#{db}_dmls".intern, 4)
      response = Kernel.system("#{CLI} dmls apply until 201108190002::test_migration -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_until_milestone_when_milestone_not_exist
    @dbs.each do |db|
      response = `#{CLI} dmls apply milestone rel1 -c #{TEMP_DIR}/#{db}.conf`
  ...
  def test_apply_until_milestone_when_milestone_not_specified
    @dbs.each do |db|
      response = `#{CLI} dmls apply milestone -c #{TEMP_DIR}/#{db}.conf`
  ...
  def test_apply_until_milestone_when_milestone_file_empty
    @dbs.each do |db|
      FileUtils.touch(File.join(TEMP_DIR, 'dml', 'rel1.milestone'))
      response = `#{CLI} dmls apply milestone rel1 -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/The milestone does not contain a valid dml migration/, response)
    end
  end

  def test_apply_until_milestone_when_non_existent_migration_in_file
    File.open(File.join(TEMP_DIR, 'dml', 'rel1.milestone'), 'w') do |f|
  ...
  def test_apply_until_milestone
    ...
    File.open(File.join(TEMP_DIR, 'dml', 'rel1.milestone'), 'w') do |f|
      f.puts '201108190002_up_test_migration.sql'
    end

    @dbs.each do |db|
      self.send("helper_many_good_#{db}_dmls".intern, 4)
      response = Kernel.system("#{CLI} dmls apply milestone rel1 -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response, db)
    end
  end
  ...
  def test_invalid_apply_command
    response = `#{CLI} dmls apply foobar -c #{TEMP_DIR}/sqlite.conf`
  ...
  def test_apply_last_rollback_when_none
    @dbs.each do |db|
      response = `#{CLI} dmls rollback last -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/There are no applied dml migrations to rollback/, response)
    end
  end
  ...
  def test_apply_last_rollback_when_some
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply next -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      response = Kernel.system("#{CLI} dmls rollback last -c #{TEMP_DIR}/#{db}.conf")
    end
  end
  ...
  def test_rollback_last_migration_with_errors_and_force
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply next -c #{TEMP_DIR}/#{db}.conf")
      self.send("helper_bad_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls rollback last -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_all_migrations_when_none
    @dbs.each do |db|
      response = `#{CLI} dmls rollback all -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/There are no applied dml migrations to rollback/, response)
    end
  end
  ...
  def test_rollback_all_migrations_when_some
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply next -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      response = Kernel.system("#{CLI} dmls rollback all -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_all_migrations_with_errors_and_force
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls rollback all -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_specific_good_migration
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      response = Kernel.system("#{CLI} dmls rollback 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_specific_bad_migration
    @dbs.each do |db|
      ...
      assert_equal(true, response)
      self.send("helper_bad_#{db}_dml".intern)
      response = `#{CLI} dmls rollback 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/There was a problem rolling back/, response)
    end
  end
  ...
  def test_rollback_specific_bad_migration_with_force
    @dbs.each do |db|
      ...
      assert_equal(true, response)
      self.send("helper_bad_#{db}_dml".intern)
      response = Kernel.system("#{CLI} dmls rollback 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_apply_specific_rollback_when_not_exist
    @dbs.each do |db|
      response = `#{CLI} dmls rollback 201108190000::not_there -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/The dml migration file .+ does not exist/, response)
    end
  end
  ...
  def test_rollback_until_good_migration
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply until 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      response = Kernel.system("#{CLI} dmls rollback until 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_until_bad_migration
    @dbs.each do |db|
      self.send("helper_many_good_#{db}_dmls".intern, 2)
      response = Kernel.system("#{CLI} dmls apply until 201108190001::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      self.send("helper_many_bad_#{db}_dmls".intern, 2)
      response = Kernel.system("#{CLI} dmls rollback until 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(false, response)
    end
  end
  ...
  def test_rollback_until_bad_migration_force_on
    @dbs.each do |db|
      ...
      assert_equal(true, response)
      self.send("helper_many_bad_#{db}_dmls".intern, 2)
      response = Kernel.system("#{CLI} dmls rollback until 201108190000::test_migration -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_until_migration_that_does_not_exist
    @dbs.each do |db|
      response = `#{CLI} dmls rollback until 201108200000::test_migration -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/has not been applied so cannot be rolledback/, response)
    end
  end
  ...
  def test_rollback_until_migration_with_invalid_name
    @dbs.each do |db|
      response = `#{CLI} dmls rollback until 2011000000::test_migration -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/is not a valid dml migration name/, response)
    end
  end
  ...
  def test_rollback_until_migration_missing_name
    @dbs.each do |db|
      response = `#{CLI} dmls rollback until -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/A dml migration name must be specified/, response)
    end
  end
  ...
  def test_rollback_until_migration_many_migrations
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply all -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      response = Kernel.system("#{CLI} dmls rollback until 201108190002::test_migration -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_until_migration_many_bad_migrations_force_on
    @dbs.each do |db|
      ...
      assert_equal(true, response)
      self.send("helper_many_bad_#{db}_dmls".intern, 4)
      response = Kernel.system("#{CLI} dmls rollback until 201108190002::test_migration -c #{TEMP_DIR}/#{db}.conf -f")
      assert_equal(true, response)
    end
  end
  ...
  def test_rollback_until_milestone_when_no_milestone_specified
    @dbs.each do |db|
      response = `#{CLI} dmls rollback milestone -c #{TEMP_DIR}/#{db}.conf`
  ...
  def test_rollback_until_milestone_when_milestone_not_exist
    @dbs.each do |db|
      response = `#{CLI} dmls rollback milestone not_here -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/The milestone .+ does not exist/, response)
    end
  end
  ...
  def test_rollback_until_milestone_when_milestone_file_empty
    @dbs.each do |db|
      FileUtils.touch(File.join(TEMP_DIR, 'dml', 'rel1.milestone'))
      response = `#{CLI} dmls rollback milestone rel1 -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/The milestone does not contain a valid dml migration/, response)
    end
  end
  ...
  def test_rollback_until_milestone_when_badly_formed_migration_in_file
    @dbs.each do |db|
      ...
      response = `#{CLI} dml rollback milestone rel1 -c #{TEMP_DIR}/#{db}.conf`
      assert_match(/The milestone does not contain a valid dml migration/, response)
    end
  end
  ...
  def test_rollback_until_milestone
    ...
    @dbs.each do |db|
      ...
      response = Kernel.system("#{CLI} dmls apply all  -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
      response = Kernel.system("#{CLI} dmls rollback milestone rel1 -c #{TEMP_DIR}/#{db}.conf")
      assert_equal(true, response)
    end
  end
...
#  end
  ...
  def test_invalid_rollback_command
    response = `#{CLI} dmls rollback foobar -c #{TEMP_DIR}/sqlite.conf`
    assert_match(/is not a valid command/, response)
  end

  def test_invalid_plugin_directory
    helper_many_good_sqlite_dmls(4)
    response = `#{CLI} dmls apply next -c #{TEMP_DIR}/sqlite_bad_plugin.conf`
